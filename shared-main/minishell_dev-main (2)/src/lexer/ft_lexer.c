...
/*   Updated: 2024/10/10 01:05:11 by hrazafia         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (ft_isspace(s[i])
		|| ft_str_compare(&s[i], LEXEME_PIPE)
		|| ft_str_compare(&s[i], LEXEME_INPUT_REDIR)
		|| ft_str_compare(&s[i], LEXEME_OUTPUT_REDIR)
		|| ft_str_compare(&s[i], LEXEME_HEREDOC)
		|| ft_str_compare(&s[i], LEXEME_APPEND))
		return (true);
	else
		return (false);
}
...
static void	ft_add_operator(t_list **lexeme, char *s, size_t *index)
{
	...
	if ((lexeme == NULL) || (s == NULL) || (index == NULL))
		return ;
	i = *index;
	if (ft_str_compare(&s[i], LEXEME_PIPE))
		operator = ft_strdup(LEXEME_PIPE);
	else if (ft_str_compare(&s[i], LEXEME_HEREDOC))
		operator = ft_strdup(LEXEME_HEREDOC);
	else if (ft_str_compare(&s[i], LEXEME_INPUT_REDIR))
		operator = ft_strdup(LEXEME_INPUT_REDIR);
	else if (ft_str_compare(&s[i], LEXEME_APPEND))
		operator = ft_strdup(LEXEME_APPEND);
	else if (ft_str_compare(&s[i], LEXEME_OUTPUT_REDIR))
